...
const Courses = () => {
    ...
    const [filteredCouses , setFilteredCouses]=useState([]);


  useEffect(() => {
    const loadCourses = async () => {
      const data = await fetchCourses();
      setFilteredCouses(data.courses)
  ...
  const handleDeleteCourse = async () => {
    try {
        ...
        if (res.ok) {
            ...
            setFilteredCouses(updatedCourses.courses)
        }
    } catch (error) {
  ...
  const handleSearch = (e) => {
    ...
    const allCourses = courses.filter((course) => {
        // Search in both course names and teacher names
