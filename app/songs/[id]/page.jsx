        ...
        <div className="w-full bg-[#F6F4EE] dark:bg-gray-900">
            ...
            <div className="container">
                <div className="w-full flex flex-col justify-center">
                    <div className="bg-mango-100 dark:bg-gray-700 rounded-3xl shadow-xl p-3 flex flex-col gap-y-5">
                        <div className="flex flex-col md:flex justify-center items-center gap-y-10 md:gap-x-10">
                            ...
                            <div className="flex flex-col gap-y-5">
                                <h2 className="text-2xl md:text-4xl font-MorabbaBold dark:text-gray-50">لیست موسیقی های آموزشــــی ساز <span className="bg-mango rounded-lg p-1 dark:text-elf">{songs.instrument_name}</span></h2>
                                <h2 className="md:text-xl font-DanaMedium dark:text-gray-50"> تعداد موسیقی {songs.songs?.length || 0} </h2>
                            </div>
                        </div>
                        {songs?.books?.map((book) => (
                            <div key={book._id} className={`w-full flex flex-col rounded-lg  ${selectedBookId === book._id ? 'gap-5' : ''}`}>
                                <div onClick={() => handleBookClick(book._id)} className={`flex justify-between items-center p-4 cursor-pointer dark:hover:bg-gray-500 md:hover:bg-slate-200 rounded-xl`}>
                                    <div className=" font-DanaMedium dark:text-gray-50">{book.book_name}</div>
                                    <div className={`${chevron?'rotate-90 transform ease-in-out dark:text-gray-50':''}`}>
                                ...
                                {selectedBookId === book._id && (
                                    <div>
                                        {book?.songs?.map((song) => (
                                            <div key={song._id} onClick={() => handleSongClick(song)} className="w-full flex flex-col dark:bg-gray-600 dark:text-gray-50 gap-y-2 p-2 cursor-pointer md:hover:bg-slate-200 font-Dana rounded-xl">
